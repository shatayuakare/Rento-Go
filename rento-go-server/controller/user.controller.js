...
export const login = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Account Loged In", token: req.session.token });
  } catch (error) {
    res.status(400).json({ message: error.message });
  }
};
export const isLoggedIn = async (req, res, next) => {
  try {
    ...
    if (!token)
      return res
        .status(403)
        .json({ message: "You have no permission, Please Login" });

    // console.log(token)
    const decode = jwt.verify(token, key);
    console.log(token);
    const user = await Users.findOne({ email: decode.user.email }).select(
      "-password"
    );
...
export const deleteUser = async (req, res) => {
  try {
    ...
    if (!isValid)
      return res.status(503).json({ message: "Unauthorized access" });

    await Users.findOneAndDelete({ email })
      .then(() => res.status(200).json({ message: "Account Deleted", user }))
...
export const forgotPassword = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: `${user.name}'s your password is ${pass}` });
  } catch (error) {
    res.status(400).json({ message: error.message });
  }
};
